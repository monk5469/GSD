...
if  isfield(DCM,'SPM') && isfield(DCM,'xY')
    ...
    [X,Xn,Fc] = spm_Volterra(DCM.U, DCM.SPM.xBF.bf, DCM.SPM.xBF.Volterra);  %%%The SPM12 path needs to be added
    DCM.U.X0  = ones(size(X,1),1);
    X         = [X(1 : r_dt : end, : ) DCM.U.X0(1 : r_dt : end)];
    for nR = 1 : NR
        
        Beta(:,nR) = DCM_Adjust_beta(nR,DCM);%After recoding, the beta values corresponding
        %Beta(:,nR) = DCM.xY(nR).Beta;   %to the 5 experimental effects
        
    end
    
    y_GLM = X(:,1:end-1) * Beta (1:end,:); % GLM output
    x     = u * Beta(1:end,:);             % neuronal activity
...
DCM.cb = [DCM.c reshape(DCM.b, [NR, NR*Nu])];
X_O    = X_O(:,:,NR);
X_O    = [X_O(:,1:end-1,:)  zeros(length(y_GLM),NR*Nu) DCM.U.X0(1 : r_dt : end)];
X_O    = repmat (X_O, [1,1,NR]);      %The design matrix is extended to bilinear terms.
...
for nR = 1 : NR
    ...
    for nu = 1: Nu
        ...
        if sum( DCM.b(nR,:,nu) ) ~= 0 % judge what input exist bi-linearity
            ...
            cCol = find ( DCM.cb(nR,Nu+1:end));
            
            for c = 1:length(bCol)
                X_O(:,NR+Nu+cCol(c),nR) = UXH(:,c);%Add bidirectional terms
                DCM.cb(nR,Nu+cCol(c))   = 0;
            end
