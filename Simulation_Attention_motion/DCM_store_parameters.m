function [ output ] = DCM_store_parameters(DCM, mN_cut, sN, aN, bN, logF, logF_term, z_cut, args)
...
Nc = size(DCM.cb, 2) - size(DCM.U.u,2);
Nu = size(DCM.U.u, 2);
% DCM.b = DCM.b(:,:,1:end-Nc);
DCM.cb = DCM.cb(:,1:end-Nc);
...
CTmp                  = mN(1:nr,end-Nc-nu+1:end-Nc);
[row,col]             = find (DCM.c) ;
output.Ep.C           = zeros (nr,Nu);
output.Ep.C (row,col) = CTmp (row,col);
CTmp(row,col)         = 0;

%-- Unpack cb matrix to matrices c and b
Nu                    = size(DCM.U.u, 2);
output.Ep.B           = zeros(nr,nr,Nu);
for nR = 1 : nr
    
   if sum(DCM.b(nR,:,:),'all') ~= 0 % judge which region exist bi-linearity

      for nu = 1: Nu
         
         if sum( DCM.b(nR,:,nu) ) ~= 0 % Judge which input exists bi-linearity
            
            bCol = find ( DCM.b(nR,:,nu) );% Find where bi-linearity is in b
            cCol = find ( CTmp(nR,:)  );% Find where bi-linearity is in c
            for b = 1:length(bCol)
                output.Ep.B(nR,bCol(b),nu) = CTmp (nR,cCol(b)); % let the bi-linearity value of b
                                                                % assigned by cb 
                CTmp (nR,cCol(b))          = 0; % update temporary cb
            end
