...
%         N_eff = sum(idx_y);
       ...
       if length(ord_regions) == 3 %%Attention data
        % The model is allowed to trim the drive input 
            %and the modulation input at the corresponding position
            clocc           = find(DCM.cb(k,1:end-1)==0);
            p0(clocc + nr ) = DCM.cb(k, clocc);
       end 
       
       if length(ord_regions) == 50  %%silmultion data
           
           % make sure that the driving inputs are only on the correct connections
        p0(end-(size(DCM.cb,2)-1):end-1) = DCM.cb(k,1:end-1);
